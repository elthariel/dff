...
class DirModelItem():
    ...
    # TAKE :        row : int
    # RETURN :  OCaseDirItemModel or False
    def childWithNode(self, nodeVFS):
        if self.childCount() < 1:
      #      print DFF_Utils.getPath(self.nodeVFS),"1",  DFF_Utils.getPath(nodeVFS)
            return False
        else :
            childItems = self.childItems
            for i in childItems:
                if i.nodeVFS.this == nodeVFS.this:
                    return i
#        print "2",  str(DFF_Utils.getPath(nodeVFS))
        return False
    
    def deleteChild(self, nodeVFS):
        if self.childCount() < 1:
            return False
        else :
            childItems = self.childItems
